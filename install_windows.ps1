...
if ($env:PATH -notlike "*Tesseract-OCR*") {
    ...
    [Environment]::SetEnvironmentVariable("PATH", "$currentUser;C:\Program Files\Tesseract-OCR", "User")
}
...
if (-not (Test-Path $popplerBin)) {
    ...
    if ($extracted) {
        Rename-Item $extracted.FullName "C:\poppler" -ErrorAction SilentlyContinue
    }
    Remove-Item $zipPath
...
if ($env:PATH -notlike "*poppler*") {
    $env:PATH += ";$popplerBin"
    $currentUser = [Environment]::GetEnvironmentVariable("PATH", "User")
    [Environment]::SetEnvironmentVariable("PATH", "$currentUser;$popplerBin", "User")
...
$src = "$projectDir\PDF OCR 실행.bat"
$dst = "$env:USERPROFILE\Desktop\PDF OCR 실행.bat"
